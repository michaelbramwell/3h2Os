// Type definitions for external libraries (Chart.js, Marked)
...
interface Window {
    trainingPlan: Week[];
    activityStore: { [key: string]: Activity };
    showDetails: (actId: string) => void;
    printWeek: (index: number) => void;
    closeModal: () => void;
}
...
function formatZonesHTML(zones: Zone[], type: 'pace' | 'hr' | 'power'): string {
...
function highlightCurrentWeek() {
    const rows = document.querySelectorAll<HTMLTableRowElement>('#plan-body tr');
...
function renderWeightChart(weightData: { current: number, target: number, history: WeightEntry[] }) {
...
function updateSidebar(data: ContextData) {
    if (!data || !data.project || !data.runner || !data.status) return;

    // Update Goal
    const goalEl = document.querySelector('.lg\\:col-span-1 > div:nth-child(1) p:nth-child(2)') as HTMLElement;
    const dateEl = document.querySelector('.lg\\:col-span-1 > div:nth-child(1) p:nth-child(3)') as HTMLElement;
    if (goalEl) goalEl.innerText = data.project.goal;
    if (dateEl) dateEl.innerText = `${data.project.event} • ${new Date(data.project.eventDate).toLocaleDateString('en-AU', { month: 'long', day: 'numeric', year: 'numeric' })}`;

    // Update Phase
    const phaseEl = document.querySelector('.lg\\:col-span-1 > div:nth-child(2) p:nth-child(2)') as HTMLElement;
    const nextActionEl = document.querySelector('.lg\\:col-span-1 > div:nth-child(2) p:nth-child(3)') as HTMLElement;
    if (phaseEl) phaseEl.innerText = data.status.phase;
    if (nextActionEl) nextActionEl.innerText = data.status.nextAction;
    ...
    if (wCurr) wCurr.textContent = `${current}kg`;
    if (wTarg) wTarg.textContent = `/ ${target}kg target`;
    if (wBar) wBar.style.width = `${progressPercent}%`;
    if (wMeta) wMeta.textContent = `${(current - target).toFixed(1)}kg to lose (${progressPercent.toFixed(0)}% progress)`;
...
function renderRecentActivities(actuals: Activity[]) {
    ...
    list.innerHTML = actuals.slice(0, 5).map(act => {
        ...
        let impact: string[] = [];
        if (act.training_load) impact.push(`L:${Math.round(act.training_load)}`);
...
function renderPlan(weeks: Week[], actuals: Activity[]) {
    ...
    weeks.forEach((week, index) => {
        ...
        ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].forEach(day => {
            ...
            const plannedHtml = dayData.workouts.map(w => {
                ...
                if (w.type === 'Intervals') color = 'text-purple-600 font-semibold';
                if (w.type === 'Threshold') color = 'text-red-600 font-semibold';
                if (w.type === 'Steady') color = 'text-blue-600 font-semibold';
                if (w.type === 'Race') color = 'text-orange-600 font-bold';
                return `
            ...
            const actualHtml = dayActuals.map(act => {
                ...
                if (!window.activityStore) window.activityStore = {};
...
async function loadDashboard() {
    try {
        ...
        const [planData, contextData, actualsData] = await Promise.all([
        ...
        if (contextData.status) {
            ...
            if (meta) meta.textContent = `Last updated: ${contextData.status.lastUpdated}`;
        }
...
window.showDetails = function(actId) {
    ...
    if (title) title.textContent = act.name;
    if (subtitle) subtitle.textContent = `${new Date(act.date).toLocaleDateString('en-AU', { weekday: 'long', day: 'numeric', month: 'long' })} • ${act.type}`;
