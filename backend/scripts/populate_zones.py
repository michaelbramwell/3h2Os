...
def populate_zones():
    with Session(engine) as session:
        ...
        pace_zones = [
            TrainingZone(
                zone=1, lowBoundary_m_s=0.0, description="Recovery. Very easy jogging."
            ),
            TrainingZone(
                zone=2, lowBoundary_m_s=2.5, description="Easy. Conversational pace."
            ),
            TrainingZone(
                zone=3, lowBoundary_m_s=2.9, description="Tempo. Marathon effort."
            ),
            TrainingZone(
                zone=4, lowBoundary_m_s=3.3, description="Threshold. Comfortably hard."
            ),
            TrainingZone(
                zone=5, lowBoundary_m_s=3.6, description="VO2 Max. Hard effort."
            ),
            TrainingZone(
                zone=6, lowBoundary_m_s=4.0, description="Anaerobic. Sprints."
            ),
        ]
