...
class ValidationEngine:
    def __init__(
        ...
        max_intensity_ratio: float = 0.25,
    ):  # Hard running shouldn't be >25% (example)
    ...
    def validate_progression(
        ...
        previous_week: Week,
        current_week: Week,
        focused_workout: Optional[Workout] = None,
        ...
        prev_vol = get_week_volume(previous_week)
        curr_vol = get_week_volume(current_week)
    ...
    def validate_structure(
        ...
        if focused_workout:
            is_focus_intensity = any(
                t in focused_workout.type.lower() for t in INTENSITY_TYPES
            )
