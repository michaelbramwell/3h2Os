...
class ActivityService:
    ...
    def save_activities(self, activities: List[ActivitySchema], username: str = "mike") -> int:
        ...
        for act in activities:
            # Check if exists
            existing = self.session.exec(select(ActualActivity).where(ActualActivity.activity_id == act.activityId)).first()
            ...
            def dump_zones(zones):
                ...
                try:
                    return json.dumps([z.model_dump() if hasattr(z, 'model_dump') else z for z in zones])
                except:
                    return None
    ...
    def get_activities(self, username: str = "mike") -> List[ActivitySchema]:
        ...
        activities = self.session.exec(select(ActualActivity).where(ActualActivity.user_id == user.id).order_by(ActualActivity.date)).all()
